...
ORDERTYPE_MT2VT: dict[int, tuple] = {
    TYPE_BUY: (Direction.LONG, OrderType.TAKER),
    TYPE_SELL: (Direction.SHORT, OrderType.TAKER),
...
class Mt5Gateway(BaseGateway):
    ...
    exchanges: list[Exchange] = [Exchange.MT5]
    ...
    def connect(self, setting: dict) -> None:
        ...
        self.query_contract()
        self.query_order(OrderQueryRequest)
    ...
    def send_order(self, req: OrderRequest) -> str:
        ...
        if req.type == OrderType.FOK or req.type == OrderType.FAK or req.type == OrderType.RFQ:
            self.write_log(f"不支持订单类型: {req.type.value}")
            return ""
        # print(cmd)
        local_id: str = self.new_orderid()
        offset = "open" if req.offset ==Offset.OPEN else "close"
        mt5_req: dict = {
        ...
        if result:
            ...
            self.write_log(f"订单 {local_id} 被拒绝，原因： {comment}")
    ...
    def cancel_order(self, req: CancelRequest) -> None:
        ...
        if req.orderid not in self.local_sys_map:
            self.write_log(f"取消订单失败，找不到的系统订单id{req.orderid}")
            return
        ...
        if result is True:
            self.write_log(f"取消订单成功{req.orderid}")
        elif result is False:
            self.write_log(f"取消订单失败: {req.orderid}")
    ...
    def query_contract(self) -> None:
        ...
        if packet:
            self.write_log("MT5被连接。")

        for d in packet["data"]:
            notional = str(d["min_lot"]).rstrip("0")
    ...
    def query_history(self, req: HistoryRequest) -> list[BarData]:
        ...
        if packet["result"] == -1:
            ...
            msg: str = f"查询kline历史记录已完成, {req.symbol.replace('.','-')} - {req.interval.value}, {begin} - {end}"
            self.write_log(msg)
                ...
                if local_id and order:
                    ...
                    if data["result_order"] and self.sys_local_map[order_id] == order_id:
                        ...
                        trade: TradeData = TradeData(
                            ...
                            price=Decimal(data["result_price"]),
                            volume=Decimal(data["result_volume"]),
                        ...
                        order.status = Status.REJECTED
                        self.write_log(f"订单 {local_id} 被拒绝，原因：market_closed")
                        self.on_order(order)
            return
        ...
        # Update trade data
            ...
            if order:
                ...
                trade: TradeData = TradeData(
                    ...
                    tradeid=data["deal"],
                    price=Decimal(data["trans_price"]),
                    volume=Decimal(data["trans_volume"]),
                    datetime=datetime.now(CHINA_TZ),
                    gateway_name=self.gateway_name
                )
                order.traded = trade.volume
                # print(f"trader :{trade}")
                self.on_order(order)
                self.on_trade(trade)
...
def generate_china_datetime(timestamp: str) -> datetime:
    """Generate China datetime"""
    dt: dict = datetime.strptime(timestamp, "%Y.%m.%d %H:%M")
